...
// safely compared by the ADC to the internal reference voltage. The resistors
// in the potential divider are nominally 1 Mohm and 499 Kohm; since there is a
// variation in the actual values, and because I am a pedant, I calculate
// V_RATIO for each device using a 3.3 V reference power supply.
#define	V_REFERENCE		1.1
#define	V_RATIO			3.0
#define	V_SCALE			(V_REFERENCE * V_RATIO)
...
loop()
{
	...
	case READING_COUNT:
		if (type == T_TEMPERATURE) {
			...
			ADCSRA |= _BV(ADEN);
			value = V_SCALE * analogRead(BATTERY_INPUT) / 1024.0;	
			ADCSRA &= ~_BV(ADEN);
		}
		wd_count = 0;
		break;
